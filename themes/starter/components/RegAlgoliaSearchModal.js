    ...
    <div
      ...
      <div
        ...
        {/* <input
          ...
          className='float-start text-black dark:text-gray-200 bg-gray-50 dark:bg-gray-600 outline-blue-500 w-full px-4 my-2 py-1 mb-4 border rounded-md'
          ref={inputRef}
        /> */}
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <div>
            {/* <a onClick={onJumpSearchResult}
                              ...
                              className={`signUpBtn text-white rounded-md bg-white bg-opacity-20 px-6 py-1  my-5 mr-6 flex items-center h-full w-24 pt-px signup-text-base font-medium duration-300 ease-in-out hover:bg-opacity-100 hover:text-dark`}
                              >
                            下一步
                              </a> */}
        ...
        <ul className='flex-1 overflow-auto'>
          {searchResults.map((result, index) => (
            <li
              ...
              rounded-lg
              ${activeIndex === index ? 'bg-blue-600 light:bg-yellow-600' : ''}`}>
              <a
                className={`${activeIndex === index ? ' text-white' : ' text-black light:text-gray-300 '}`}>
                {result.title}
              </a>
            </li>
          ))}
        ...
        <div className='flex items-center justify-between mt-2 sm:text-sm text-xs light:text-gray-300'>
          {totalHit === 0 && (
            <div className='flex items-center'>
              {ShortCutActions.map((action, index) => {
                return (
                  <Fragment key={index}>
                    <div className='border-gray-300 light:text-gray-300 text-gray-600 px-2 rounded border inline-block'>
                      {action.key}
                    </div>
                    <span className='ml-2 mr-4  text-gray-600 light:text-gray-300'>
                      {action.action}
                    </span>
                  </Fragment>
                )
          ...
          <div className='text-gray-600 light:text-gray-300  text-right'>
            <span>
...
function TagGroups() {
  ...
  return (
    <div id='tags-group' className='light:border-gray-700 space-y-2'>
      {firstTenTags?.map((tag, index) => {
        return (
          <Link
            ...
            <div
              className={
                'flex items-center text-black light:text-gray-300 hover:bg-blue-600 light:hover:bg-yellow-600 hover:scale-110 hover:text-white rounded-lg px-2 py-0.5 duration-150 transition-all'
              }>
  ...
  return (
    <div className='flex space-x-1 w-full justify-center py-1'>
      {Array.from({ length: totalPage }, (_, i) => {
        const classNames =
          page === i
            ? 'font-bold text-white bg-blue-600 light:bg-yellow-600 rounded'
            : 'hover:text-blue-600 hover:font-bold light:text-gray-300'
